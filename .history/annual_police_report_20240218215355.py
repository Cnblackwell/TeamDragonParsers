...
def hello_world():
    # Plot and save Arrests by Year graph
    # Assume `instance_class` is defined and contains necessary data
    by_year = instance_class.by_year_f(instance_class.total_only_f(annual_police_report))
    instance_class.year_column(by_year, 'arrest_count')
    plt.savefig('arrests_by_year.png')

def cool_beans():
    # Plot and save Arrests By Age graph
    year_age_df = instance_class.arrest_by_year(total_df)
    arrest_year = year_age_df['arrest_year']
    instance_class.arrest_by_age_year(year_age_df, arrest_year)
    plt.savefig('arrests_by_age.png')

def tong():
    # Plot and save Arrests, Cohort by Year graph
    instance_class.arrest_cohort(annual_police_report)
    plt.savefig('arrests_cohort_by_year.png')

def tong2():
    # Plot and save Relationship between Age and Year graph
    instance_class.linear_regression(annual_police_report)
    plt.savefig('relationship_age_year.png')
